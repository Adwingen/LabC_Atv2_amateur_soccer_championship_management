...
void update_table(char* home_team, int home_goals, char* away_team, int away_goals) 
{
    
 
    int home_index = find_team(home_team);
    if (home_index == -1) 
    {
        home_index = add_team(home_team);
    }
    int away_index = find_team(away_team);
    if (away_index == -1) 
    {
        away_index = add_team(away_team);
    }
...
void read_game_data(char* filename) 
{
    ...
    while (fgets(line, 1024, file)) 
    {
        ...
        if (home_team != NULL && home_goals_str != NULL && away_team != NULL && away_goals_str != NULL) 
        {
            ...
            update_table(home_team, home_goals, away_team, away_goals);
        }
    }
    fclose(file);
}
